...
const EconomicsTopics = () => {
  ...
  const documentTitle = 'Learnr | Economics Articles'

  const link = useRef()

  const getEconomics = Object.values(JSON.parse(localStorage.getItem('economicsArticles')))
  console.log(getEconomics)
  console.log(getEconomics.length)
  const arr = []
  const articleBody = (item) => getEconomics.forEach((body) => {
    arr.push(body)
  })
  articleBody(link)
  console.log(arr)
    ...
    <React.Fragment>
      ...
      <div className="container">
        <div className="topics-wrap">
          <h3 className="topic-subject">{`Economics - ${getEconomics.length} Topics`}</h3>
            ...
            <div className="topics-list">
              <div className="">
                ...
                <span className=" d-flex flex-column" >
                  {getEconomics.map((getTopics, index) => {
                    return (
                      <div ref={link} className="topics-item " key={index}>
                        <span onClick={() => {
                          ...
                          localStorage.setItem('learnrEconomicsArticleBody', JSON.stringify(article))
                          window.open('/economics/article', '_self')
                        }
