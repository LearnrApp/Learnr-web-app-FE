...
const MathematicsTopics = () => {
  ...
  const documentTitle = 'Learnr | Mathematics Articles'

  const link = useRef()

  const getMathematics = Object.values(JSON.parse(localStorage.getItem('mathematicsArticles')))
  console.log(getMathematics)
  console.log(getMathematics.length)
  const arr = []
  const articleBody = (item) => getMathematics.forEach((body) => {
    arr.push(body)
  })
  articleBody(link)
  console.log(arr)
    ...
    <React.Fragment>
      ...
      <div className="container">
        <div className="topics-wrap">
          <h3 className="topic-subject">{`Mathematics - ${getMathematics.length} Topics`}</h3>
            ...
            <div className="topics-list">
              <div className="">
                ...
                <span className=" d-flex flex-column" >
                  {getMathematics.map((getTopics, index) => {
                    return (
                      <div ref={link} className="topics-item " key={index}>
                        <span onClick={() => {
                          ...
                          localStorage.setItem('learnrMathematicsArticleBody', JSON.stringify(article))
                          window.open('/mathematics/article', '_self')
                        }
